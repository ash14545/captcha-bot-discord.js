...
`);
    ...
    if (commandfile.maintenance && ![client.developer.id].includes(message.author.id)) return message.channel.send('This command is still being worked on, please check back later...')

    if (commandfile.category.toLowerCase() == 'developer' && ![client.developer.id].includes(message.author.id)) return message.channel.send('This command is for the developer only!')

    for (const perm of commandfile.clientPermissions) {
        if (!message.channel.permissionsFor(client.user.id).has(perm)) return message.channel.send(`
...
`)
    ...
    if (![client.developer.id, client.user.id].includes(message.author.id)) {
        if (![].includes(command)) {
...
client.on("message", async message => {
    ...
    var member = member.guild.members.cache.get(message.author.id);
    let verifiedRole = '';
    let unverifiedRole = '';
    ...
    if (captcha[message.author.id] == message.content) {
        member.roles.remove(unverifiedRole);
        member.roles.add(verifiedRole);

        message.channel.send('You have successfully verified yourself!');
    } else {
        tries -= 1;

        if (tries > 0) {
            message.channel.send(`Sorry that is incorrect, try again! *You have ${tries} more attempt${tries == 1 ? '' : 's'} left.*`);
        } else {
            message.channel.send('Sorry, you failed human verification and will be removed from the server.');
            member.kick('Failed human verification');
        }
    }
